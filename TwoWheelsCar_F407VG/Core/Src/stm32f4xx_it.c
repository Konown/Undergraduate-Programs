...
void USART2_IRQHandler(void)
{
  ...
  /* USER CODE BEGIN USART2_IRQn 1 */
    if (__HAL_UART_GET_FLAG(&huart2, UART_IT_IDLE) != RESET){
        ...
        Uart2_Str.Uart_RecvLens = UART_BUFFSIZE - __HAL_DMA_GET_COUNTER(&hdma_usart1_rx);
        HAL_UART_Transmit_DMA(&huart1, Uart2_Str.Uart_RecvBuff, Uart2_Str.Uart_RecvLens);
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *UartHandle, uint16_t Size) {
    if (UartHandle == &huart1) {
        ...
        //重启接收
        HAL_UART_Transmit_DMA(&huart1, (uint8_t *)Uart2_Str.Uart_RecvBuff, Size);
        HAL_UARTEx_ReceiveToIdle_DMA(&huart2, Uart2_Str.Uart_RecvBuff, sizeof(Uart2_Str) - 1);
        __HAL_DMA_DISABLE_IT(&hdma_usart2_rx, DMA_IT_HT);

        Uart2_Str.Uart_RecvLens = Size;
        Uart2_Str.flag_new = 1;
    }
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
//        HAL_UART_Transmit_DMA(&huart1, (uint8_t *)Uart3_Str.Uart_RecvBuff, 1);
        Uart3_Str.Analysis_function();
        HAL_UART_Receive_IT(&huart3, Uart3_Str.Uart_RecvBuff, 1);
    }
